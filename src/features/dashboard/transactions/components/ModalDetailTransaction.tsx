...
const ModalDetailTransaction: FC<ModalDetailTransactionProps> = ({
  ...
  const { data: tx, isPending: pendingTxDetail } = useGetTransactionDetail(
    transaction.uuid,
  );
  const { mutateAsync: accept, isPending: pendingAccept } =
    useAcceptTransaction();
  const { mutateAsync: reject, isPending: pendingReject } =
  ...
  const handleAccept = async () => {
    await accept(transaction.uuid);
  };

  const handleReject = async () => {
    await reject(transaction.uuid);
  };

  if (pendingTxDetail || !tx) {
    return (
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[600px]">
        ...
        <DialogFooter>
          {tx.status && tx.status === "WAITING_CONFIRMATION" && (
            <div className="space-x-2">
              <ModalAcceptPayment
                onClick={handleAccept}
                isPending={pendingAccept}
              >
                Accept Payment
              </ModalAcceptPayment>
              <ModalRejectPayment
